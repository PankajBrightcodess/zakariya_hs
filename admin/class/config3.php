...
class Database {
	...
	private $_host = "db709810779.db.1and1.com";
	private $_username = "dbo709810779";
	private $_password = "Bms@123#";
	private $_database = "db709810779";
	/*
	...
	*/
	...
	function login($table,$username,$password){			
		...
		$query.="(`mobile` = '".$username."' or `email` = '".$username."') and `password` = '".$password."' and `active` = '1'";
		$run=$this->_connection->query($query);
		if($run){
			if($run->num_rows==1){
				$array=$run->fetch_assoc();
	...
	//Bulk SMS
	/** SenderId for Bulk sms */
	$senderId="BOOKMS";
	
	/** serverUrl for Bulk sms */
	$serverUrl="sms.rsgss.com";
	
	/** Authentication Key for Bulk sms */
	$authKey="91de4016e1fece462ab6d94c1171aa1";
	
	/** sms Route for Bulk sms */
	$route="1";
?>