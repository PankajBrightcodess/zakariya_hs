...
function saveContact($name,$mobileNumber,$city,$occ,$email,$serverUrl,$authKey)
{
	$groupid='16510';
	$getData='name='.urlencode($name).'&mobileNumber='.$mobileNumber.'&groupId='.$groupid.'&city='.urlencode($city).'&occupation='.urlencode($occ).'&emailId='.$email;
    //$getData = 'mobileNos='.$mobileNumber.'&message='.urlencode($message).'&senderId='.$senderId.'&routeId='.$routeId;

    //API URL
    $url="http://".$serverUrl."/rest/services/phonebook/addContact?AUTH_KEY=".$authKey."&".$getData;
